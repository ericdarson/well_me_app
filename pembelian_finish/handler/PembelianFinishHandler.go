...
import (
	"pembelian_finish_api/dao"
	"pembelian_finish_api/entity/response"

	"github.com/gin-gonic/gin"
)

var (
	daoFinishPembelian dao.PembelianFinishDao = dao.NewPembelianFinish()
)

func PembelianFinish() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		var JSONReponse response.PembelianFinishResponse
		...
		if vendorPw == nil {
			...
			outputSchema := daoFinishPembelian.DoneBeli(vendorPw[0], idTrans)
			...
			} else if outputSchema == "SYSTEM ERROR" || outputSchema[0:6] == "GAGAL" {
